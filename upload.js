(() => {
  ...
  try {const u=new URL(config.intakeUrl);validEndpoint=u.protocol==='https:'&&u.hostname.endsWith('.netlify.app')&&u.pathname==='/api/intake'&&!u.username&&!u.password&&!u.search&&!u.hash;}catch{}
  ...
  function snapshot(){const data=new FormData();const id=crypto.randomUUID();for(const name of ['full_name','email','client_reference','period','document_type','note','invite_code'])data.append(name,form.elements[name].value.trim());data.append('schema_version','1');data.append('submission_id',id);data.append('submitted_at',new Date().toISOString());data.append('requirement_complete',String(form.elements.requirement_complete.checked));data.append('test_mode','true');data.append('file_count','1');data.append('file_1',files.files[0],files.files[0].name);return {id,data};}
  async function send(){if(busy||!pending||!enabled)return;busy=true;button.disabled=true;fields.disabled=true;result.hidden=true;button.textContent='שולחים את החומרים…';const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),60000);
    try {const response=await fetch(config.intakeUrl,{method:'POST',body:pending.data,signal:controller.signal,credentials:'omit',redirect:'error'});const raw=await response.text();let receipt;try{receipt=JSON.parse(raw);}catch{}
